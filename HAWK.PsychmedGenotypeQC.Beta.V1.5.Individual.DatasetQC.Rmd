...
miscfolder="/home/c.c1002680/Pathfinder/GenoQCPipeExtra"
...
system(paste(GH," --input ",outpath,"/",DATASET,".qc0.clean --ref ",miscfolder,"/HRC.r1-1.GRCh37.wgs.mac5.sites.vcf.gz --output ",outpath,"/",DATASET,".gh --ambiguousSnpFilter --update-id --mafAlign 0.1 -I PLINK_BED -R VCF",sep=""))
...
#iii) Checks for total number of SNPs removed by GH. If 2/3 of SNPs are lost after harmonisation, the pipeline continues otherwise runs liftover to update build from hg18 to hg19 and creates new bed/bim/fam
 if(nrow(bim.gh)/nrow(bim)<=(1/3)){
  warning(paste("Over 2/3 of SNPs have been removed by Genome Harmoniser. Applying liftover to convert from hg18 to hg19.",sep=""))
  ...
  if(file.exists(paste(outpath,"/",DATASET,".ped",sep=""))==F){
    system(paste(PLINK," --allow-extra-chr --allow-no-sex --bfile ",outpath,"/",DATASET, " --recode --out ",outpath,"/",DATASET,sep=""))
  ...
  system(paste(liftover," -e ",miscfolder,"/liftOver -c ",miscfolder,"/hg18ToHg19.over.chain.gz -m ",DATASET,".map -p ",DATASET,".ped -o ",DATASET,".grc37",sep=""))
    system(paste(PLINK," --allow-extra-chr --allow-no-sex --file ",outpath,"/",DATASET,".grc37 --make-bed --out ",outpath,"/",DATASET,".grc37",sep=""))
    system(paste(GH," --input ",outpath,"/",DATASET,".grc37 --ref ",miscfolder,"/HRC.r1-1.GRCh37.wgs.mac5.sites.vcf.gz --output ",outpath,"/",DATASET,".gh --ambiguousSnpFilter --update-id --mafAlign 0.1 -I PLINK_BED -R VCF",sep=""))
    GH.REPORT=as.data.frame(fread(input=paste(outpath,"/",DATASET,".gh_snpLog.log",sep=""),header=T,stringsAsFactors = F))
    GH.EXCLUDE=GH.REPORT[which(GH.REPORT$action=="Excluded"),]
    ...
    if(nrow(bim.gh)/nrow(bim)<=(1/3)){
    warning(paste("Over 2/3 of SNPs have still been removed by Genome Harmoniser. The genome build is not hg18. Converting from HG38 to HG19.",sep=""))
    
system(paste(liftover," -e ",miscfolder,"/liftOver -c ",miscfolder,"/hg38ToHg19.over.chain.gz -m ",DATASET,".map -p ",DATASET,".ped -o ",DATASET,".grc37",sep=""))
    system(paste(PLINK," --allow-no-sex --allow-extra-chr --file ",outpath,"/",DATASET,".grc37 --make-bed --out ",outpath,"/",DATASET,".grc37",sep=""))
    system(paste(GH," --input ",outpath,"/",DATASET,".grc37 --ref ",miscfolder,"/HRC.r1-1.GRCh37.wgs.mac5.sites.vcf.gz --output ",outpath,"/",DATASET,".gh --ambiguousSnpFilter --update-id --mafAlign 0.1 -I PLINK_BED -R VCF",sep=""))
    GH.REPORT=as.data.frame(fread(input=paste(outpath,"/",DATASET,".gh_snpLog.log",sep=""),header=T,stringsAsFactors = F))
    GH.EXCLUDE=GH.REPORT[which(GH.REPORT$action=="Excluded"),]
    write.table(GH.EXCLUDE$id,file=paste(outpath,"/",DATASET,".GH.HRC.Excluded.SNPs.PostLiftover.txt",sep=""),col.names=F,quote=F,row.names=F)
    bim.gh=read_table2(paste0(outpath,"/",DATASET,".gh.bim"), col_names=c("CHR","SNP","cm","BP","A1","A2"))
    if(nrow(bim.gh)/nrow(bim)<=(1/3)){
    stop(paste("Over 2/3 of SNPs have still been removed by Genome Harmoniser. Genome build is not hg18,hg19 or hg38.",sep=""))
...
if(length(grep("0-HGDP",pca.aim1$ID))>0){
pca.aim1[grep("0-HGDP",pca.aim1$ID),]$ID=gsub("0-HGDP","HGDP",pca.aim1[grep("0-HGDP",pca.aim1$ID),]$ID)
}

if(length(grep("-",grep("SAGP",pca.aim1$ID,value=T)))>0){
pca.aim1[grep("SAGP",pca.aim1$ID),]$ID=do.call(rbind,strsplit(pca.aim1[grep("SAGP",pca.aim1$ID),]$ID,"-"))[,1]
}
...
lda.test.aim1$IID <- rownames(lda.test.aim1)

if(class(fam$IID)=="numeric" & class(lda.test.aim1$IID)=="character"){
fam$IID=as.character(fam$IID)
}

lda.test.aim1 <- left_join(subset(fam,select=c("FID","IID")),lda.test.aim1,by="IID")
...
```{r 15vii, include=F,eval=F}
## AIM analysis 2: Eurasian/Mediterranean biogeographical ancestry inference 

#This analysis is based on the set of Ancestry Informative Markers (AIMs) contained in the SWA (PMID: [29625264](https://www.sciencedirect.com/science/article/pii/S1872497318301704)) panel. Originally, this contains a total of 86 AIMs, but these have been restricted based on availability in the `r paste0(DATASET.NAME)` dataset.
plink_aim1_data <- paste0(PLINK," --bfile ",DATASET,".qc4.clean --keep ",DATASET,".eur_swa.aim1.ids --make-bed --out ",DATASET,".aim2")
plink_aim1_ref <- paste0(PLINK," --bfile ",miscfolder,"/",REF.PANEL," --keep ",REF.PANEL,".eur_swa.ids --make-bed --out ",REF.PANEL,".aim2")
system(plink_aim1_data, ignore.stderr = T)
system(plink_aim1_ref, ignore.stderr = T)
plink_aim1_merge <- paste0(PLINK," --bfile ",DATASET,".aim2 --bmerge ",REF.PANEL,".aim2 --make-bed --out ",DATASET,".ref2 --geno 0.01")
system(plink_aim1_merge, ignore.stderr = F) # If strand errors occur, they'll appear in the console

snpgdsBED2GDS(bed.fn=paste0(DATASET,".ref2.bed"),bim.fn=paste0(DATASET,".ref2.bim"),fam.fn=paste0(DATASET,".ref2.fam"),out.gdsfn=paste0(DATASET,".ref2.gds"))
### Load GDS for KING
genodata <- snpgdsOpen(filename=paste0(DATASET,".ref2.gds"))
### Estimate relatedness using KING-robust method
kindata <- snpgdsIBDKING(genodata)
colnames(kindata$kinship) <- kindata$sample.id
rownames(kindata$kinship) <- kindata$sample.id
...
```{r 15viii, include=F,eval=F}
# Read in data for the global AIM set and its tags (ordered by r2)
eurasian.tags <- read_table2(paste(miscfolder,"/Bulbul2018_86AIM.tags.list",sep=""),na="NONE")
eurasian.tags$TAG1 <- eurasian.tags$SNP # Add each SNP as its first tag
eurasian.tags2 <-  read.table(text=eurasian.tags$TAGS, sep = "|", colClasses = "character", fill=T, na.strings = c("", "NA"), col.names=paste0("TAG",2:max(str_count(eurasian.tags$TAGS,"\\|")+2,na.rm=T)))
eurasian.tags <- cbind(subset(eurasian.tags,select="SNP"),subset(eurasian.tags,select="TAG1"),eurasian.tags2)
rm(eurasian.tags2)
# Organise data as a sorted dataframe
eurasian.tags <- melt(eurasian.tags,id.vars="SNP",value.name="tagsnp",variable.name="tagorder",na.rm=T)
eurasian.tags <- eurasian.tags[with(eurasian.tags,order(SNP,tagorder)),]
eurasian.tags$tagorder <- gsub("[^0-9.-]", "", eurasian.tags$tagorder)
# Select SNPs present in the data and reference (or closest tag)
eurasian.tags$geno <- (eurasian.tags$tagsnp %in% bim$SNP & eurasian.tags$tagsnp %in% ref1_bim$SNP)
eurasian.tags <- subset(eurasian.tags,geno==T)
eurasian.tags <- eurasian.tags[match(unique(eurasian.tags$SNP), eurasian.tags$SNP),]
eurasian.snps <- unique(eurasian.tags$tagsnp)
write.table(eurasian.snps, paste0(DATASET,".aim2.snp"), quote=F,col.names = F,row.names=F)
rm(eurasian.tags)

# Merge input and reference data retrieving only the AIMs
plink_aim1_data <- paste0(PLINK," --bfile ",DATASET,".qc4.clean --extract ",DATASET,".aim2.snp --keep ",DATASET,".eur_swa.aim1.ids --make-bed --out ",DATASET,".aim2")
plink_aim1_ref <- paste0(PLINK," --bfile ",miscfolder,"/",REF.PANEL," --extract ",DATASET,".aim2.snp --keep ",REF.PANEL,".eur_swa.ids --make-bed --out ",REF.PANEL,".aim2")
system(plink_aim1_data, ignore.stderr = T)
system(plink_aim1_ref, ignore.stderr = T)
plink_aim1_merge <- paste0(PLINK," --bfile ",DATASET,".aim2 --bmerge ",REF.PANEL,".aim2 --make-bed --out ",DATASET,".ref2")
system(plink_aim1_merge, ignore.stderr = F) # If strand errors occur, they'll appear in the console
```

```{r 15ix, include=FALSE,eval=F}
# Transform merged PLINK file into GDS file
snpgdsBED2GDS(bed.fn=paste0(DATASET,".ref2.bed"),bim.fn=paste0(DATASET,".ref2.bim"),fam.fn=paste0(DATASET,".ref2.fam"),out.gdsfn=paste0(DATASET,".ref2.gds"))
# Load GDS for PC-Air
genodata <- GdsGenotypeReader(paste0(DATASET,".ref2.gds"))
genodata <- GenotypeData(genodata) 
# Generate PCs for LDA
pcdata <- pcair(genodata,kinobj=kindata$kinship,divobj=kindata$kinship,unrel.set=ref.aim1.eur_swa$IID,eigen.cnt=max.aimpcs) # Base PCs on the reference panel
pca.aim2 <- as.data.frame(pcdata$vectors)
close(genodata)
```

```{r 15x, fig.align="center",eval=F}
# Add sample and variable names to PCA frame
colnames(pca.aim2) <- paste0("PC",c(1:max.aimpcs))
pca.aim2$ID <- rownames(pca.aim2)
# Restrict to Tracy-Widom significant PCs or the first 5 (if fewer than 5)
pca.aim2.tw <- tw(pcdata$values,max.aimpcs,criticalpoint = 0.9793)$SigntEigenL
if (pca.aim2.tw < 5) {pca.aim2.tw <- 5}
rm(pcdata)
# Merge reference population data with PCs
pca.aim2 <- left_join(pca.aim2,ref1.pop,by="ID")
rownames(pca.aim2) <- pca.aim2$ID
pca.aim2$Population[is.na(pca.aim2$Population)] <- DATASET.NAME
pca.aim2$Region[is.na(pca.aim2$Region)] <- DATASET.NAME 
pca.aim2$Region <- factor(pca.aim2$Region, levels=c("Middle_East","Europe","Central_South_Asia",DATASET.NAME), labels=c("North African","European","Southwest Asian",DATASET.NAME))
pca.aim2 <- with(pca.aim2, pca.aim2[order(Region,ID),]) # Sort for plotting purposes
# Plot ancestries
pca.aim2.p1 <- ggplot(pca.aim2, aes(x = PC1, y= PC2,fill=Region))+
  geom_point(aes(shape=Region),alpha = 0.75,size = 3, colour="black") +
  scale_shape_manual("",values=c(rep(21,3),22)) +
  scale_fill_manual("",values=c("firebrick","blue","purple","white")) +
  labs(title="",y="PC2",x="PC1") +
  theme_fivethirtyeight(base_family="sans") +
  theme(plot.title = element_blank(),
        plot.background = element_rect(fill = "white"),
        ...
        legend.position="top",
        legend.direction="horizontal")
pca.aim2.p2 <- ggplot(pca.aim2, aes(x = PC3, y= PC4,fill=Region))+
  geom_point(aes(shape=Region),alpha = 0.75,size = 3, colour="black") +
  scale_shape_manual("",values=c(rep(21,3),22)) +
  scale_fill_manual("",values=c("firebrick","blue","purple","white")) +
  labs(title="",y="PC4",x="PC3") +
  ...
  theme(plot.title = element_blank(),
      plot.background = element_rect(fill = "white"),
        axis.title=element_text(),
        legend.position="none")
pca.aim2.legend <- get_legend(pca.aim2.p1 + theme(legend.background = element_rect(fill = "white")))
pca.aim2.title <- ggdraw() + draw_label(paste("PCA on",length(eurasian.snps),"AIMs for Eurasian/Mediterranean biogeographical inference"), fontface='bold')
pca.aim2.p1p2 <- plot_grid(pca.aim2.p1+theme(legend.position="none"), pca.aim2.p2,
           align = 'vh', hjust = -1, nrow = 1, rel_widths = c(1, 1))
plot_grid(pca.aim2.title,pca.aim2.legend, pca.aim2.p1p2, ncol=1, rel_heights=c(0.15,0.1, 1)) 
```


```{r 15xi,eval=F}
#The LDA algorithm has been applied to the first `r paste0(pca.aim2.tw)` principal components of the HGDP dataset. Model training statistics for the prediction of each individual biogeographical ancestry using the available `r length(eurasian.snps)` AIMs are shown below.


# Create and train LDA model
pca.aim2.test.data <- subset(pca.aim2,Region == DATASET.NAME,select=paste0("PC",c(1:pca.aim2.tw)))
pca.aim2.train.data <- subset(pca.aim2,!Region == DATASET.NAME,c("Region",paste0("PC",c(1:pca.aim2.tw))))
pca.aim2.train.class <- str_replace_all(as.character(pca.aim2.train.data$Region)," ","_") # Replace spaces with underscores
pca.aim2.train.data$Region <- NULL
lda.train.aim2 <- train(x=pca.aim2.train.data,y=pca.aim2.train.class, method="lda", metric="Accuracy", trControl=trainControl(method="cv", number=10, classProbs = T, savePredictions = T))
# Output results of LDA model
lda.train.cm <- confusionMatrix(lda.train.aim2$pred$obs,lda.train.aim2$pred$pred)
lda.train.cm <- lda.train.cm$byClass[,c("Sensitivity","Specificity","Pos Pred Value","Neg Pred Value","Balanced Accuracy")]
rownames(lda.train.cm) <- gsub("Class: ", "", rownames(lda.train.cm)) # Remove the string "Class:"
rownames(lda.train.cm) <- str_replace_all(rownames(lda.train.cm),"_"," ") # Bring the spaces back
lda.train.cm[] <- sprintf("%.2f%%", 100 * lda.train.cm) # Replace numbers with percentages
# Plot table and reorder for consistency with PCA plot
kable(lda.train.cm[c("North African","European","Southwest Asian"),]) %>%
  kable_styling(bootstrap_options = c("striped"))
# A threshold for ancestry inference has been set at `r percent(min.prob.aim2)` probability. Any individual not achieving this probability for any ancestry has been labelled _"Unclassified"_. Full results of the LDA model, including probabilities for each ancestry, have been outputted to the file _`r paste0(DATASET,".aim_eurasian.prob.txt")`_.
```
...
```{r 15xii,eval=F}
# Predict ancestries in the input data
lda.test.aim2 <- predict(object=lda.train.aim2,newdata=pca.aim2.test.data,type="prob")
lda.test.aim2 <- round(lda.test.aim2,3) # Restrict output to three decimal places
lda.test.aim2$IID <- rownames(lda.test.aim2)
lda.test.aim2 <- right_join(subset(fam,select=c("FID","IID")),lda.test.aim2,by="IID")
# Report predicted ancestries
lda.test.aim2$Ancestry <- "Unclassified"
lda.test.aim2$Ancestry[lda.test.aim2$European >= min.prob.aim2] <- "European"
lda.test.aim2$Ancestry[lda.test.aim2$North_African >= min.prob.aim2] <- "North African"
lda.test.aim2$Ancestry[lda.test.aim2$Southwest_Asian >= min.prob.aim2] <- "Southwest Asian"
lda.test.aim2$Ancestry <- factor(lda.test.aim2$Ancestry, levels=c("North African","European","Southwest Asian","Unclassified"))
lda.test.aim2.table <- as.data.frame(table(lda.test.aim2$Ancestry))
colnames(lda.test.aim2.table) <- c("Ancestry","Samples")
kable(lda.test.aim2.table) %>% kable_styling(full_width = F, bootstrap_options = c("striped"))
write.table(lda.test.aim2,file=paste0(DATASET,".aim_eurasian.prob.txt"),col.names=T,row.names=F,quote=F,sep="\t")


lda.test.aim.final <- left_join(lda.test.aim.final,subset(lda.test.aim2,select=c("FID","IID","Ancestry")),by=c("FID","IID"))
colnames(lda.test.aim.final) <- c("FID","IID","Global_model","Eurasian_model")
lda.test.aim.final$Eurasian_model=as.character(lda.test.aim.final$Eurasian_model)
lda.test.aim.final$Eurasian_model[is.na(lda.test.aim.final$Eurasian_model)] <- "Not applicable"
#For most purposes, including GWAS, the "Global" ancestries will be accurate enough. Some unclassified individuals might be resolved by looking at the results of the "Eurasian" ancestries, and more stringent ancestry definitions can be achieved by combining the results of both "Global" and "Eurasian" models. For those individuals with a clear discrepancy between the two reported ancestries, inspection of the raw classification probabilities from both models is reccomended. 
```
...
**Non-QC'd Y Chr:** _`r if(file.exists(paste0(DATASET,".Y.bed/bim/fam"))==T){paste0(DATASET,".Y.bed/bim/fam")} else if(file.exists(paste0(DATASET,".Y.bed/bim/fam"))==F){paste0("No Y Chr SNPs were present")}`_  
