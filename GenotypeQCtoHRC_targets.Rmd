---
title: "Genotype QC to HRC ¦ `r params$dataset`"
...
http_address <- glue("file://{file_path}")
cat("[A list of markers with non-standard chromosomes is available here.](", http_address, ")")
```
...
http_address <- glue("file://{file_path}")
cat("[A list of markers with A/B allele codes is available here.](", http_address, ")")
```
...
tar_load(bfile.qc3,strict=F)
if (nrow(bfile.nonauto$y)==0){ # No Y data
qc3.ind.dataset <- mutate(qc3.ind.dataset,
         YRATE=NA,
         potential.aneuploidy=NA)}} else {
...
rm(qc4.pcrelate) # Cleanup (tends to be a very large object)
...
if (object.size(phiframe) <= 8589934592) { # Sets a 8GB limit for plotting the data, creates alternative (smaller) plot if it goes over threshold
...
phiframe.p1 } else {
phiframe.sum <- summarise(group_by(subset(phiframe,relatedness != "Unrelated"),relatedness),pairs=n())
phiframe.p1 <- ggplot(data=phiframe.sum, aes(y = pairs, x = relatedness, fill=relatedness)) +
  geom_bar(stat = "identity") +
  scale_fill_discrete_sequential(palette="Batlow", rev=F, drop=T,nmax=6) +
  labs(title="PC-Relate kinship distribution",
       subtitle="Dataset too large to render main plot, summary plot with related pairs shown instead",
       y="Pairs of samples",x="Kinship category",fill="") +
  theme(axis.title=element_text(),
        legend.position="right",
        legend.direction = "vertical")
phiframe.p1 }
